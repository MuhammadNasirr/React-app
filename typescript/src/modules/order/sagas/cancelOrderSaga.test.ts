...
import { getOrders, ordersData } from '../actions';
...
describe('Admin activity', () => {
    ...
    describe('Fetch admin activity data', () => {
        ...
        const fakeFetchPayloadFirstPage = { page: 1, limit: 2, state: 'wait' };
        ...
        const expectedActionsFetchWithFirstPage = [
            getOrders(fakeFetchPayloadFirstPage),
            ordersData(fakeSuccessPayloadFirstPage),
        ];
        ...
        it('should fetch user activity for 1 page in success flow', async () => {
            ...
            store.dispatch(getOrders(fakeFetchPayloadFirstPage));
            return promise;
        });
    });
});
