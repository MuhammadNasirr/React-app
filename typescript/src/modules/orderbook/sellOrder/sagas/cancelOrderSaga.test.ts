...
import { getSellOrders, sellOrdersData } from '../actions';
...
describe('Admin activity', () => {
    ...
    describe('Fetch admin activity data', () => {
        ...
        const expectedActionsFetchWithFirstPage = [
            getSellOrders(fakeFetchPayloadFirstPage),
            sellOrdersData(fakeSuccessPayloadFirstPage),
        ];
        ...
        it('should fetch user activity for 1 page in success flow', async () => {
            ...
            store.dispatch(getSellOrders(fakeFetchPayloadFirstPage));
            return promise;
        });
    });
});
