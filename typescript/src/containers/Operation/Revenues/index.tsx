...
import {
    ...
    AppState,
    getRevenues,
    RevenuesDataInterface,
    selectRevenues,
    selectRevenuesCurrentPage,
    selectRevenuesLoading,
    selectRevenuesTotalNumber,
} from '../../../modules';
...
interface ReduxProps {
    ...
    total: number;
    page: number;
    revenues: RevenuesDataInterface[];
}

interface DispatchProps {
    getRevenues: typeof getRevenues;
...
class RevenuesScreen extends React.Component<Props, AssetsState> {
    ...
    public componentDidMount() {
        ...
        this.props.getRevenues({ page: page + 1, limit: rowsPerPage, ordering: 'desc' });
    }
    ...
    private handleChangePage = (page: number) => {
        ...
        this.handleGetRevenues(this.state.rowsPerPage, page);
    };
    ...
    private handleChangeRowsPerPage = (rows: number) => {
        ...
        this.handleGetRevenues(rows, 0);
    };

    private handleGetRevenues = (limit: number, page: number) => {
        this.props.getRevenues({ limit, page: page + 1, ordering: 'desc' });
    }
...
const mapStateToProps: MapStateToProps<ReduxProps, {}, AppState> =
    (state: AppState): ReduxProps => ({
        revenues: selectRevenues(state),
        loading: selectRevenuesLoading(state),
        total: selectRevenuesTotalNumber(state),
        page: selectRevenuesCurrentPage(state),
    });

const mapDispatchToProps: MapDispatchToPropsFunction<DispatchProps, {}> =
    dispatch => ({
        getRevenues: params => dispatch(getRevenues(params)),
        alertPush: params => dispatch(alertPush(params)),
    });

const RevenuesPage = connect(mapStateToProps, mapDispatchToProps)(RevenuesScreen);

// tslint:disable-next-line:no-any
export const Revenues = withRouter(RevenuesPage);
